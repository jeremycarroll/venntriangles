...
typedef uint32_t COLOR;    /* Actually between 0 and 5, with NO_COLOR as a
                              potential (I think unused). */
...
struct edge {
  ...
  STATIC FACE outer;
  /*
  We assign both points when we use this edge in the facial cycle of either
  inner or outer face. We assign one point if we assign this edge to it, prior
  to it being used in a facial cycle.
  */
  DYNAMIC POINT from;
  DYNAMIC POINT to;
  STATIC COLOR color;
...
extern POINT createPoint(EDGE aEdgeIn, EDGE aEdgeOut, EDGE bEdgeIn,
