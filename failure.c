...
static struct failure MultipleFailure = {0, "Multiple failures", {0}, MultipleFailures};
...
FAILURE maybeAddFailure(FAILURE failureCollection, FAILURE newFailure, int depth)
{
    ...
    case CROSSING_LIMIT_FAILURE_TYPE:
        MultipleFailures[CountOfFailuresInMultipleFailure++] = *(COLORSET *)newFailure->moreInfo;
        break;
...
static struct failure CrossingLimitFailure = {CROSSING_LIMIT_FAILURE_TYPE, "More than 3 crossing points", {0}, &CrossingLimitColorPair};
