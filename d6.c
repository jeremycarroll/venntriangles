                                  ...
                                  UseFaceDegrees callback)
{
  ...
  for (int i = NCOLORS; i >= 3; i--) {
    args[depth] = i;
    canoncialCallbackImpl(depth + 1, sum + i, args, callback);
  }
}

void canoncialCallback(UseFaceDegrees callback)
{
  int args[NCOLORS];
  canoncialCallbackImpl(0, 0, args, callback);
}
