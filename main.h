...
extern FACE_DEGREE CentralFaceDegreesFlag[NCOLORS]; /* Central face degrees (-d) */
...
extern int PerFaceDegreeMaxSolutionsFlag;  /* Per-degree max solutions (-m with -d) */
extern int GlobalSkipSolutionsFlag;        /* Global solutions to skip (-k) */
extern int PerFaceDegreeSkipSolutionsFlag; /* Per-degree solutions to skip (-k with -d) */
extern int IgnoreFirstVariantsPerSolution; /* Variants to ignore per solution (-j) */

/* Program entry points */
extern int dynamicMain0(int argc, char* argv[]); /* Main program entry */
extern struct predicate* NonDeterministicProgram[]; /* Search algorithm predicates */
