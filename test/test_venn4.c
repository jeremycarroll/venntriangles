...
//   TEST_ASSERT_TRUE(d6PermutationEqual(identity, d6Compose(identity,
//   identity)));
// }
...
//   TEST_ASSERT_TRUE(d6PermutationEqual(identity, d6Identity()));
//   s4a =
//       d6Closure(&size, 2, d6Permutation(1, 0, 2, 3), d6Permutation(1, 2, 3,
//       0));
//   TEST_ASSERT_EQUAL(24, size);
//   s4b = d6Closure(&size, 3, d6Permutation(1, 0, 2, 3),
//                   d6Permutation(2, 1, 0, 3), d6Permutation(3, 1, 2, 0));
//   TEST_ASSERT_EQUAL(24, size);
//   TEST_ASSERT_EQUAL(0, memcmp(s4a, s4b, 24 *
//   sizeof(((PERMUTATION)NULL)[0]))); for (int i = 0; i < size; i++) {
//     TEST_ASSERT_TRUE(
//         d6PermutationEqual(identity, d6Compose(&s4a[i],
//         d6Inverse(&s4a[i]))));
//   }
// }
